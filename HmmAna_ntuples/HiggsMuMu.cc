...
int main(int argc, char* argv[])
{
  ...
  */
   ...
   proc_scale["VBFHToMuMu_2016"]=0.000823*lumi_16/1315771.54443;//8342013.06055;
   ...
   proc_scale["ZH_2016"]=0.000192*lumi_16/187722.110596;//413010.311768;
   proc_scale["WplusH_2016"]=0.000183*lumi_16/105952.702148;//360018.046387;//105908.515625;
   proc_scale["WminusH_2016"]=0.000116*lumi_16/225130.591797;//158522.949219;//66702.0078125;
   ...
   proc_scale["ttH_2016"]=0.000110*lumi_16/193219.242188;//258743.374512;//258860.732483;
   ...
   proc_scale["DYJetsToLL_VBFfilter_2016"]=2.02*lumi_16/2573466407.0;//2570758278.5;
   ...
   proc_scale["DYJetsToLL_M105To160_incl_2016"]=46.9479*lumi_16/6831848279.44;//7223964604.28;//(2699711552.0);
   ...
   proc_scale["ttTo2l2v_2016"]=87.31*lumi_16/3291868029.41;//79083400.0;//64203460.0;
   proc_scale["ttsl_2016"]=6.871e+02*lumi_16/18472653875.5;//150930664.0;
   ...
   proc_scale["EWK_incl_2016"]= 1.608*lumi_16/485017.0;
   proc_scale["EWK_2016"]= 0.0508896*lumi_16/9148064.0;//1576200.0;
   proc_scale["EWK_2017"]= 0.0508896*lumi/3676958.2182;//3678099.0;
   proc_scale["EWK_2018"]= 0.0508896*lumi_18/1978654.78125;//1978878.0;
   proc_scale["ggH_2018"]=0.010571*lumi_18/216906552.0;
   proc_scale["ggH_2017"]=0.010571*lumi/381714429.875;
   proc_scale["ggH_2016"]=0.010571*lumi_16/55415900.0;//996618.0;//58565928.4755;
   ...
   proc_scale["WZTo3LNu_2016"]=4.42965*lumi_16/96793125.0625;//97457043.1172;//95696911.5444;
   proc_scale["WZTo2L2Q_2016"]=5.595*lumi_16/233192814.125;//76043637.625;//211724236.125;
   ...
   proc_scale["ZZTo4L_2016"]=1.256*lumi_16/103121112.0;//82437095.0;//82474616.0;//5728400.0;
   proc_scale["ZZTo2L2Q_2016"]=3.22*lumi_16/496436.0;
   proc_scale["ZZTo2L2Nu_2016"]=0.564*lumi_16/57586850.0;//8931750.0;//7530975.0;
   ...
   proc_scale["WWToLNuQQ_2016"]=4.599e+01*lumi_16/8654600.0;//1999200.0;
   ...
   proc_scale["WWW_4F_2016"]=0.2086*lumi_16/50075.4301758;//50012.1679688;//50040.8632812;
   proc_scale["WZZ_2016"]=0.05565*lumi_16/13734.3183594;//13735.4013062;//11341.4560547;
   proc_scale["ZZZ_2016"]=0.01398*lumi_16/3491.11474609;//3499.77664185;//3500.18438721;
   ...
   proc_scale["DYJetsToLL_2016"]=2075*3*lumi_16/1.85101576666e+12;//1.86045481216e+12;//1856607046400.000000000;//1.8938682816e+12;
   ...
   proc_scale["TTWJetsToLNu_2017"]=0.2001*lumi/1402612.79297;//1680699.34375;
   proc_scale["TTZToLLNuNu_2016"]=0.2529*lumi_16/3005261.06787;//1473794.00171;
...
void HiggsMuMu::Categorization(const char *data,const char *isData, float mlo, float mhi, double scale, TString procname)
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	 ...
	 if(i!=t_mu1 && i!=t_mu2){
	   if(t_Mu_pt->at(i)>20. && t_Mu_miniPFRelIso_all->at(i)<0.4 && t_Mu_sip3d->at(i) < 8. && t_Mu_dxy->at(i) < 0.05 && t_Mu_dz->at(i) < 0.10) mu.push_back(i);
	 }
       }  
     }
	 ...
	 if(t_El_miniPFRelIso03_all->at(i)<0.4 &&  t_El_pt->at(i)>20. && fabs(t_El_eta->at(i))<2.4 && t_El_cutBased->at(i)==2 && t_El_sip3d->at(i) < 8. && t_El_dxy->at(i) < 0.05 && t_El_dz->at(i) < 0.10){
	   el.push_back(i);
	 }
       }  
     }
	  ...
	  if(*isData=='F'){
	    ...
	    if(procname == s6 || procname == s9){ // 2016 samples with Tune CUETP8M1 have different Z pt reweighting
	  ...
	  */
	    ...
	    if(diMuon_mass>108.){ //In the sidebands
	      if(*isData=='F'){
	    ...
	    cthetaCS=2*(reco_mu2.E()*reco_mu1.Pz()-reco_mu1.E()*reco_mu2.Pz())/(diMuon_mass*sqrt(pow(diMuon_mass,2)+pow(diMuon_pt,2)));
