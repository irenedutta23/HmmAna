...
void DatavsMC_31July_h_sideband_postfit()
{
  ...
  */
  ...
  for(int j=0;j<2;j++){
    directory_md0[j] = (TDirectory*) Main_Dir[j]->Get("ch5");
  
  
  }
  TString chname[33];
  
  chname[0]= "h_sideband_dijet_inv_mass_postfit";
  chname[1]= "h_sideband_pt_balance_postfit";
  /*
  chname[0]= "h_sideband_dnn_pred_postfit";
  chname[1]= "h_sideband_pnn_pred_postfit";*/
  ...
  for(int j=0;j<2;j++){
    ...
    TAxis *xaxis_temp=h25[j]->GetXaxis();
    int nbins_temp=h25[j]->GetNbinsX();
    float xlow=xaxis_temp->GetBinLowEdge(1);
    float xup=xaxis_temp->GetBinUpEdge(nbins_temp);
    ...
    for (Int_t i=1; i<=n; i++) {
      if(h[j]->GetBinContent(i)!=0){
	bin_err =sqrt(h_covar[j]->GetBinContent(i,i))/h[j]->GetBinContent(i);
	cout<<i<<","<<bin_err<<","<<sqrt(h_covar[j]->GetBinContent(i,i))<<endl;
  ...
  vFrame[0]->GetXaxis()->SetRangeUser(4,10);
  hd[0]->GetXaxis()->SetTitle("Mjj (GeV)");
  int nbins = hd[0]->GetXaxis()->GetNbins();
  double new_bins[nbins+1];
  for(int i=0; i <= nbins; i++){
    new_bins[i] = 100.*( hd[0]->GetBinLowEdge(i+1) );
  }
  h_err[0]->SetBins(nbins, new_bins);
  hd[0]->SetBins(nbins, new_bins);
  hd[0]->GetXaxis()->SetRangeUser(400.,1000.);
  ...
  nbins = hd[1]->GetXaxis()->GetNbins();
  new_bins[nbins+1];
  for(int i=0; i <= nbins; i++){
    new_bins[i] = 0.5*( hd[1]->GetBinLowEdge(i+1) );
  }
  h_err[1]->SetBins(nbins, new_bins);
  hd[1]->SetBins(nbins, new_bins);
  hd[1]->GetXaxis()->SetRangeUser(0.,5.);
  ...
  */
  for(int j=0;j<2;j++){
    ...
    pad2[j]->cd();
    if (j==0){
      l[j]=new TLine(400.,1.0,1000.,1.0);
    }
    else l[j]= new TLine(hd[j]->GetXaxis()->GetXmin(),1.0,hd[j]->GetXaxis()->GetXmax(),1.0);
