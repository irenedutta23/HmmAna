...
void DatavsMC_31July_z_peak_postfit()
{
  ...
  */
  ...
  plotname[2] = "dy_1j";
  plotname[3] = "dy_2j";
  plotname[4] ="ewk_lljj_mll50_mjj120";//ewk_lljj_mll50_mjj120";//
  plotname[5] = "ggh";
  ...
  for(int j=0;j<2;j++){
    directory_md0[j] = (TDirectory*) Main_Dir[j]->Get("ch4");
  
  
  }
  TString chname[33];
 
  chname[0]= "z_peak_dijet_inv_mass_postfit";
  chname[1]= "z_peak_pt_balance_postfit";
  //chname[0]= "z_peak_dnn_pred_prefit";
  // chname[1]= "z_peak_dnn_pred_prefit";
  ...
  for(int j=0;j<2;j++){
    ...
    TAxis *xaxis_temp=h23[j]->GetXaxis();
    int nbins_temp=h23[j]->GetNbinsX();
